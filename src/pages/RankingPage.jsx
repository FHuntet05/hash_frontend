// RUTA: frontend/src/pages/RankingPage.jsx (RECONSTRUIDO CON NUEVA UI)
...
// Subcomponente para una fila del ranking
const RankingItem = ({ rank, user, score }) => {
    ...
    return (
        <div className="flex items-center p-3 bg-card-hover rounded-lg">
            <div className={`w-10 text-lg font-bold flex-shrink-0 ${rankColorClass}`}>
                #{rank}
            </div>
            <img 
            ...
            <div className="text-right flex-shrink-0">
                ...
                <p className="text-xs text-text-secondary">{t('rankingPage.usdtLabel', 'USDT')}</p>
            </div>
        </div>
    );
};

// Subcomponente para la tarjeta de resumen del usuario
const UserSummaryCard = ({ summary }) => {
    ...
    return (
        <div className="bg-card p-4 rounded-lg border-2 border-accent-primary shadow-accent-glow">
...
const RankingPage = () => {
  ...
  const TabButton = ({ tabName, label }) => (
    <button 
        ...
        className={`w-1/2 py-3 text-sm font-bold transition-colors duration-300 border-b-2 ${activeTab === tabName ? 'text-accent-primary border-accent-primary' : 'text-text-secondary border-transparent hover:text-text-primary'}`}
    >
      {label}
    </button>
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex-grow">
        {loading ? ( <div className="pt-8"><Loader text={t('rankingPage.loading', 'Cargando ranking...')} /></div> ) 
         : error ? ( <div className="text-center text-status-danger p-8">{error}</div> ) 
         : ( 
            <div className="space-y-2">
                {rankingData.ranking.length > 0 ? rankingData.ranking.map((item, index) => (
                    ...
                    <div className="text-center text-text-secondary pt-8">{t('rankingPage.noData', 'No hay datos disponibles en este ranking.')}</div>
                )}
            </div>
          )}
      </div>
