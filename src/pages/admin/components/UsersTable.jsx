// RUTA: frontend/src/pages/admin/components/UsersTable.jsx (CON COLUMNA DE REFERIDOS)

import React from 'react';
import { Link } from 'react-router-dom';
import { HiBadgeCheck, HiPencil, HiOutlineShieldExclamation, HiOutlineShieldCheck, HiOutlineCurrencyDollar } from 'react-icons/hi';
            ...
            <tr key={user._id} className={`border-b border-dark-primary hover:bg-white/5 ${user.status === 'banned' ? 'bg-red-900/20' : ''}`}>
              <th scope="row" className="px-6 py-4 font-medium text-white whitespace-nowrap">
                <Link to={`/admin/users/${user._id}/details`} className="flex items-center gap-3 group">
                  ...
                  <div>
                    <div className="group-hover:text-accent-start transition-colors">{user.username}</div>
