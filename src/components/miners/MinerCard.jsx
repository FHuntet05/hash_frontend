...
// RUTA: frontend/src/components/factories/MinerCard.jsx (v4.2 - "QUANTUM LEAP": DISEÑO COMPACTO A MITAD DE TARJETA)
...
const MinerCard = ({ miner, onBuyClick, isOwned }) => {
  ...
  return (
    <motion.div
        layout
        className="relative rounded-2xl overflow-hidden shadow-lg shadow-black/30 border border-white/10 aspect-[3/4] min-h-[400px]"
    >
        {/* --- CAPA DE FONDO (IMAGEN) --- */}
        {/* La imagen ocupa todo el fondo, sirviendo como el lienzo principal. */}
        <div
        ...
        {/* Este panel ahora comienza desde la mitad de la tarjeta hacia abajo, como solicitó. */}
        <div 
            className="absolute bottom-0 left-0 right-0 h-3/4 p-5 flex flex-col justify-between 
                       bg-black/50 backdrop-blur-xl rounded-t-2xl border-t border-white/10"
        >
            {/* --- SECCIÓN DE INFORMACIÓN (COMPACTA) --- */}
            <div>
                <h3 className="text-2xl font-bold text-white shadow-text">{miner.name}</h3> 
                
                {/* Etiqueta "Adquirida" con estilo refinado */}
                {isOwned && (
            ...
            <div>
                ...
                <button 
                    ...
                    className={`
                        ...
                        ${isOwned 
                            ? 'bg-gray-700/80 cursor-not-allowed' // Botón deshabilitado como en la referencia
